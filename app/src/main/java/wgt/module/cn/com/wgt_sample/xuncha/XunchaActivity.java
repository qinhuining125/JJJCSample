...
import wgt.module.cn.com.wgt_sample.adapter.XunchaAdapter;
...
public class XunchaActivity extends AppCompatActivity implements XunchaContract.View {
...
//    RelativeLayout taskFinishtask;
        ...
        public void onLoadMore() {
            refresh.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    // 加载数据。
                    dataDialog.show();
                    mPresenter.getXunchaList(new Gson().toJson(new TaskListRequestEntity(currentPage, new TaskListRequestEntity.TaskListRequestSonEntity(taskType, startTime, endTime, area))));
                }
            }, 1000);
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_xunchatask);
    ...
    private void onClickListen() {
        taskList.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                Intent intent = new Intent(XunchaActivity.this, XunchaMessageActivity.class);
    ...
    private void initView() {
        ...
        adapter = new XunchaAdapter(dataList, this);
        // 审批。
        adapter.setTaskSp(new XunchaAdapter.SP() {
            @Override
            public void SL(String id) {
                spDialog.show();
                mPresenter.getXunchaSL(new Gson().toJson(new TaskSLRequestEntity(id)));
            }
...
//                fullXunchaFinish = new FullXunchaFinish(XunchaActivity.this);
//                fullXunchaFinish.show();
//                fullXunchaFinish.setBJDATA(new FullXunchaFinish.BJJG() {
//                    @Override
//                    public void getBJ(String data) {
//                        spDialog.show();
//                        mPresenter.getXunchaBJ(new Gson().toJson(new TaskBJRequestEntity(id, data)));
//                    }
//                });
//            }
        });
...
//        }
    ...
    @OnClick({R.id.task_starttime, R.id.task_endtime, R.id.task_area, R.id.task_menu, R.id.yask_arealayout, R.id.task_qx, R.id.task_cz, R.id.task_qd, R.id.task_back, R.id.task_alltask, R.id.task_waittask, R.id.task_worktask})
...
//        taskFinishtaskline.setVisibility(View.GONE);
    ...
    @Override
    public void setXuncha(boolean isSuccess) {
        if (isSuccess) {
            onRefrush();
            newTask.dismiss();
    ...
    @Override
    public void setXunchaSL(boolean isSuccess) {
        if (isSuccess) {
            onRefrush();
    ...
    @Override
    public void setXunchaZP(boolean isSuccess) {
        if (isSuccess) {
            onRefrush();
            zpTask.dismiss();
...
//    @Override
//    public void setXunchaBJ(boolean isSuccess) {
//        if (isSuccess) {
//            onRefrush();
...
//    }
    ...
    @Override
    public void setXunchaList(TaskEntity data) {
    ...
    public void setPersonNEW(List<PersonEntity> dataList) {
        newTask = new FullNewXuncha(this, dataList);
        newTask.show();
        // 新指派任务回调
        newTask.setTask(new FullNewXuncha.Task() {
            @Override
            public void addTask(String data, List<String> receiveId, List<String> receiveRoleId) {
                spDialog.show();
//                mPresenter.addTask(new Gson().toJson(new TaskSubmitRequestEntity(data, receiveId, receiveRoleId)));
            }
        });
    }
    ...
    public void setPersonZP(List<PersonEntity> dataList, final String id, String text) {
        ...
        zpTask.setZPTask(new FullZPXuncha.ZPTask() {
            ...
            public void zpTask(String receiveId, String receiveRoleId) {
                spDialog.show();
                mPresenter.getXunchaZP(new Gson().toJson(new TaskZPRequestEntity(id, receiveId, receiveRoleId)));
            }
        });
    }
